...
export default {
  data() {
    return {
      ...
      // 支付是否显示
      tableMixin_isShowPayForm: false,
      ...
      Dialog({
        ...
        async onConfirm() {
          try {
            ...
            await self.tableMixin_api_delTableItem({ id })
            // 删除的是当前条件的最后一条数据且不在第一页,那么页码减1
            if (self.tableMixin_tableData.length === 1 && self.tableMixin_query.page > 1) {
