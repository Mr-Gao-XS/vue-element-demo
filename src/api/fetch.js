...
const toLoginOnce = options => {
  if (['/login/google-auth', '/login/index'].indexOf(options.url) === -1) {
    router.push('/login')
    // reject('无效token')
    return false
  } else {
...
export default function (options) {
  return new Promise((resolve, reject) => {
    ...
    if (options.headers) {
      createOption.headers = options.headers
    }
    try {
      if (sessionStorage.getItem('accessToken')) {
        createOption.headers['Authorization'] = 'Bearer ' + sessionStorage.getItem('accessToken')
      } else {
        if (!toLoginOnce(options)) {
          return
        }
      }
    } catch (error) {
      ...
      error => {
        ...
        if ([401, 403].indexOf(response.status) !== -1) {
          ...
          if (response.status === 404) {
            ...
            if (error.message.includes('timeout') || JSON.stringify(response) === '{}') {
              text = '网络错误，请稍后重试'
